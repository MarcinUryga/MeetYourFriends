package com.example.marcin.meetfriends.utils

/**
 * Created by marci on 2017-11-14.
 */
object Constants {

  const val FIREBASE_ORGANIZER_ID = "organizerId"
  const val FIREBASE_EVENTS = "events"
  const val FIREBASE_USERS = "users"
  const val FIREBASE_PARTICIPANTS = "participants"
  const val FIREBASE_PARTICIPANT = "participant"
  const val FIREBASE_CHAT = "chat"
  const val FIREBASE_DATE_QUESTIONNAIRE = "dateQuestionnaire"
  const val FIREBASE_VENUE_QUESTIONNAIRE = "venueQuestionnaire"
  const val FIREBASE_QUESTIONNAIRE = "questionnaire"
  const val FIREBASE_VENUES = "venues"
  const val FIREBASE_EVENT_DESCRIPTION = "description"
  const val FIREBASE_FINISHED_VOTING = "finishedVoting"
  const val FIREBASE_ID = "id"
  const val FIREBASE_EVENT_DATE = "date"
  const val FIREBASE_EVENT_VENUE = "venue"

  const val EVENT_ID = "eventId"
  const val EMPTY_VALUE = " "
}